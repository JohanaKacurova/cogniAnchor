...
import DigitalPainting from '../modules/core/components/DigitalPainting';
...
export default function CalmZoneScreen() {
  ...
  const handleTabPress = (route: string) => {
    ...
    if (route === '/calm-zone' || route === '/contacts' || route === '/edit-schedule' || route === '/' || route === '/memory-lane' || route === '/profile' || route === '/puzzle-colors' || route === '/puzzle-levels' || route === '/puzzle-patterns' || route === '/puzzle-sizes' || route === '/record-thought' || route === '/schedule' || route === '/settings' || route === '/shape-puzzle' || route === '/add-contact') {
      router.push(route);
    }
  };
    ...
    <SafeAreaView style={[styles.container, getCalmModeStyles()]}> 
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerContent}>
          ...
          <Text style={[styles.headerSubtitle, { color: calmMode ? '#B0B0B0' : currentTheme.colors.primary }]}>Choose something peaceful to help you feel calm and centered</Text>
        </View>
      </View>
      <Animated.ScrollView 
      ...
      >
        ...
        <Animated.View style={[
          ...
          <Text style={[styles.breathingText, { color: getCalmModeTextColor() }]}>Breathe with the gentle rhythm</Text>
        </Animated.View>
        {/* Audio Section */}
        <View style={styles.audioSection}>
          <Text style={[styles.sectionTitle, { color: getCalmModeTextColor() }]}>🎶 Listen to Something Soothing</Text>
          {/* Category Buttons */}
          <View style={styles.categoryButtons}>
            ...
            >
              ...
              <Text style={[
                ...
                { color: selectedCategory === 'nature' ? '#FFFFFF' : (calmMode ? '#A0A0A0' : currentTheme.colors.text) }
              ]}>Nature Sounds</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[
            ...
            >
              ...
              <Text style={[
                styles.categoryButtonText,
                { color: selectedCategory === 'music' ? '#FFFFFF' : (calmMode ? '#A0A0A0' : currentTheme.colors.text) }
              ]}>Soft Music</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[
            ...
            >
              ...
              <Text style={[
                styles.categoryButtonText,
                { color: selectedCategory === 'messages' ? '#FFFFFF' : (calmMode ? '#A0A0A0' : currentTheme.colors.text) }
              ]}>Messages from Loved Ones</Text>
            </TouchableOpacity>
          </View>
          {/* Audio Tracks */}
          {selectedCategory && (
              ...
              {filteredTracks.map((track) => (
                ...
                >
                  ...
                  <View style={styles.trackInfo}>
                    <Text style={[styles.trackTitle, { color: getCalmModeTextColor() }]}>{track.title}</Text>
                    <Text style={[styles.trackDescription, { color: calmMode ? '#A0A0A0' : currentTheme.colors.textSecondary }]}>{track.description}</Text>
                    <Text style={[styles.trackDuration, { color: calmMode ? '#B0B0B0' : currentTheme.colors.primary }]}>{track.duration}</Text>
                  </View>
          ...
          {isPlaying && (
            <View style={styles.volumeControl}>
              <VolumeX size={scaleText(20)} color={calmMode ? '#A0A0A0' : currentTheme.colors.textSecondary} strokeWidth={2} />
        ...
        <View style={styles.visualSection}>
          <Text style={[styles.sectionTitle, { color: getCalmModeTextColor() }]}>🎨 Watch or Try Something Gentle</Text>
        ...
        <View style={styles.comfortTipSection}>
          <View style={styles.comfortTipCard}>
            <View style={styles.comfortTipHeader}>
              ...
              <Text style={[styles.comfortTipTitle, { color: getCalmModeTextColor() }]}>Gentle Reminder</Text>
              <TouchableOpacity
            ...
            <Text style={[styles.comfortTipText, { color: calmMode ? '#B0B0B0' : currentTheme.colors.textSecondary }]}>{currentTip}</Text>
          </View>
        </View>
      </Animated.ScrollView>
      {/* Digital Painting Modal */}
      {showPainting && <DigitalPainting onClose={() => setShowPainting(false)} />}
      {/* Bottom Navigation */}
...
const createStyles = (theme: any, scaleText: (size: number) => number, calmMode: boolean, currentTextScale: any) => StyleSheet.create({
  ...
  categoryButtons: {
    flexDirection: currentTextScale.id === 'extra-large' ? 'column' : 'row',
    flexWrap: currentTextScale.id === 'extra-large' ? 'nowrap' : 'wrap',
    justifyContent: 'center',
    alignItems: 'center',
    gap: scaleText(12),
  ...
  categoryButton: {
    ...
    minWidth: currentTextScale.id === 'extra-large' ? '85%' : scaleText(140),
  },
